...
const HomePrograms = ({data}: any) => {
  ...
  const programIntesectObserver = IntersectObserver(programRef);
  const [programs] = useState(data.programs || null);  

  return (
  <div className="container bg-white px-12 py-24 mx-auto">
    <div ref={programRef} className={`flex flex-wrap w-full mb-20 flex-col items-center text-center ${programIntesectObserver ? "animate-fade-up animate-once animate-ease-in" : "invisible"}`}>
    ...
    <div className="flex flex-wrap m-4">
      {
        programs.data.cards.map((item: any) => (
          <div  key={item.id} className="xl:w-1/2 md:w-1/2 p-4">
            <Card ref={programRef} className={`border border-gray-200 p-8 rounded-lg shadow-xl ${programIntesectObserver ? "animate-fade-down animate-ease-in" : "invisible"}`}>
